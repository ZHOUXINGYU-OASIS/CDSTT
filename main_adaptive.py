...
from module_ekf import ekf, sekf, dstt_sekf, cdstt_sekf, mdstt_sekf, adaptive_cdstt_sekf, adaptive_dstt_sekf
...
from module_default_settings import MIU_EM, UNIT_L, UNIT_V, SEED, STD, INITIAL_STATE, INITIAL_COVARIANCE, \
    NAV_TIMES, DIM_R, RelTol, AbsTol, N_JOBS, EPS_DSTT, EPS_CDSTT
...
def one_run_simulation(
        ...
        mu: float = MIU_EM,
        method: str = "DSTT_SEKF",
        adaptive_strategy: int = 1,
        dim_R: int = DIM_R,
    ...
    if method == "DSTT_SEKF":
        ...
        error, time_costs, cov_data, md_data, eigenvalues, eigenvectors, orders = adaptive_dstt_sekf(
        ...
        error, time_costs, cov_data, md_data, eigenvalues, eigenvectors, orders = adaptive_cdstt_sekf(
            x0r=x0r,  # true orbit state
            x0e=x0e,  # estimated orbit state
            t_series=t_series,  # time series
            P0=P0,  # initial covariance
        ...
        raise ValueError("No such method. Please select from DSTT_SEKF/CDSTT_SEKF")
    """Save data"""
    if if_save is True:
        if method == "DSTT_SEKF":
            filepath = f"./data/one_results_adaptive_{method}_{dim_R}.mat"
        else:
            filepath = f"./data/one_results_adaptive_{method}_{dim_R}_{adaptive_strategy}.mat"
    ...
    """Return results"""
    return error, time_costs, cov_data, md_data, orders
...
if __name__ == '__main__':
    ...
    method_lists = ["DSTT_SEKF", "CDSTT_SEKF"]
    for method in method_lists:
        if method == "DSTT_SEKF":
            strategy_lists = [0]  # any number
        else:
            strategy_lists = [1, 2]
        for adaptive_strategy in strategy_lists:
            error, time_costs, cov_data, md_data, orders = one_run_simulation(
                x0r=x0r,  # true orbit state
                x0e=x0e,  # estimated orbit state
